...
The first argument to `lm()` is a **formula**, written with the tilde `~`. Read the tilde as "modeled as a function of": `Sale_Price ~ Gr_Liv_Area` means "predict `Sale_Price` using `Gr_Liv_Area`." The target always goes on the left of the `~` and the features go on the right. Two extensions you'll see soon:

-   Multiple features are joined with `+`, e.g. `y ~ x1 + x2` (next section).
-   `y ~ .` is shorthand for "use *all* other variables in the data as features."

This formula syntax isn't specific to `lm()`. The same pattern reappears in `glm()`, `caret`, `tidymodels`, and most modeling functions in R, so it's worth getting comfortable with it now.
:::

For example, if we want to predict sale price (`Sale_Price`) based on house square footage (`Gr_Liv_Area`), we can use the following code:
