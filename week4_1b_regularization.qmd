...
```{r}
#| eval: false
dat <- read.csv("https://zackfisher.github.io/PSYC559/data/depression.csv")

str(dat[, 1:5])
```

-   `symptoms` is the outcome, the score at the end of the semester.
-   The next 30 columns are the predictors, named as they are on these slides.
-   `split` records which students are training and which are test, so your results match what you see here.

```{r}
#| eval: false
train_df <- subset(dat, split == "train", select = -split)
test_df  <- subset(dat, split == "test",  select = -split)
