...
-   The subset of examples used to train the learning algorithm.

-   We estimate the model's **parameters** in the training sample.

-   If we have data from 150 participants: 
    -  keep data from 100 to train the model.

-   Using **too much of our data** for training is not good. 
-   Using **too little of our data** for training is not good. 
...
-   For example, from our 100 individuals in the training sample:

    -  Split the training data into 4 groups of 25
    -  Each of the 4 groups will be used as a validation sample, after training on the others

:::  {style="color: lightblue"}
### Test Sample
:::

-   The test sample used to evaluate model performance

-    The remaining 50 individuals kept out of training: separate from training and validation. 

-    The test sample is kept completely seperate from the training and validation data. 
...
Courses often have a similar three way split:

-   **Homework** is your training data. You learn from it and you get the answers back.
-   **The practice exam** is your validation data. You use it to figure out what to study next.
-   **The final** is your test data. You see it once, and it decides your grade.

Now discuss two things that could go wrong:

1.  Why wouldn’t we want the final exam to be the thing that tells us what to study?
2.  What would be the problem with taking the same practice exam over and over until you could get every question right?
...
Naftali Harris built an interactive visualization of *k*-means where you place your own points, choose *k*, and then step through the algorithm one iteration at a time.

[Visualizing K-Means Clustering](https://www.naftaliharris.com/blog/visualizing-k-means-clustering/)

Two things to try, both of which are hyperparameter choices you just read about:

-   Run the same data with a different number of clusters.
-   Choose the starting points badly on purpose and watch where the algorithm ends up.
:::
...
1.  You have data from 10,000 participants. Do you still need a test set?
2.  You split your data, fit a model, and test accuracy comes out *higher* than training accuracy. What could explain that?
:::
...
In groups of 3 or 4 (about 6 minutes). Think of a situation outside statistics that involves learning something and then being evaluated on it. Interpret this loosely.

Then map it onto the three samples:

-   What plays the role of the **training** data?
-   What plays the role of the **validation** data?
-   What plays the role of the **test** data?

Each group shares its scenario and says which of the three was hardest to identify.
:::
...
2. Use the remaining *k* minus 1 folds as the training set.\
...
::: {style="color: yellow"}
# A Note on Randomness and Reproducibility: `set.seed()`
:::

Notice that every splitting procedure above starts by *randomly* shuffling or sampling the data. Randomness creates a practical problem: if you and I split the same dataset, we get different splits, and different results. Run it twice yourself and you'll get two answers:

```{r}
sample(1:10, 3) # randomly draw 3 numbers
sample(1:10, 3) # run it again... different draw
```

R's random numbers are actually *pseudo*-random: they come from a sequence determined by a starting point called the **seed**. If we set the seed ourselves, the "random" results become exactly reproducible:
...
set.seed(559)
sample(1:10, 3) # same seed, same draw
```

This is why nearly every machine learning script you'll see this semester begins with `set.seed()` before splitting data or fitting models that involve randomness. It doesn't make the analysis less random in spirit; it makes your random choices *repeatable*, so your results can be checked by others (including the you of next week, and me when grading).
